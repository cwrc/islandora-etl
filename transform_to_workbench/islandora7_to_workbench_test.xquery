...
<csv>
  {
    ...
    let $main_file := $metadata/media_exports/media[@ds_id/data() eq tHelper:get_main_file_from_cModel($cModel)]/@ds_id/data()
    let $associated_files := $metadata/media_exports/media[@ds_id/data() != $main_file or not(exists($main_file))]
    ...
    return
        <record>
            ...
            <a1>{tHelper:get_main_file_from_cModel($cModel)}</a1>
            <file>{$main_file}</file>
            {
